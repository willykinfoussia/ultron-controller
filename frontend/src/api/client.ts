...
export async function getCompanyNews(symbol: string) {
  return request<CompanyNewsResponse>(
    `/api/stocks/${encodeURIComponent(symbol)}/news`
  );
}

export async function getAnalysisMethods(symbol: string) {
  return request<{ symbol: string; methods: AnalysisMethod[] }>(
    `/api/analysis/${encodeURIComponent(symbol)}/methods`
  );
}

export async function runFullAnalysis(symbol: string) {
  return request<AnalysisRunResponse>(
    `/api/analysis/${encodeURIComponent(symbol)}/all`,
    { method: "POST" }
  );
}
